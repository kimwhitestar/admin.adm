...
public class AdminGuestListCommand implements AdminInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int totalRecordSize = dao.totRecCnt();//목록의 총 레코드 갯수//관리자의 경우 level=0, 관리자가 아닐 경우 level=99
		...
		//한 페이징에 표시할 레코드 검색
		List<GuestVO> vos = dao.searchGuestList(paging.getStartIndexNo(), paging.getPageSize());//관리자의 경우 level=0, 관리자가 아닐 경우 level=99
